...
import { AlertTriangle, Bell, Calendar, Edit, Plus, Power, PowerOff, Trash2 } from "lucide-react";
...
export default function AlertsPage() {
  ...
  return (
    <div className={`container mx-auto px-4 py-6 ${direction === "rtl" ? "rtl" : "ltr"}`}>
      <div className="space-y-6">
        ...
        <div className="flex gap-2 border-b border-border">
          {[
            ...
            { key: "active", label: t("alerts.active"), count: alerts.filter(a => a.isActive && !a.isTriggered).length },
            { key: "triggered", label: t("alerts.triggered"), count: alerts.filter(a => a.isTriggered).length },
            { key: "inactive", label: t("alerts.inactive"), count: alerts.filter(a => !a.isActive).length },
          ].map(({ key, label, count }) => (
            <Button
              key={key}
          ...
          <Card className="border-border/50">
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Create your first alert to get notified about important economic events.
              </p>
              <Link href="/alerts/create">
                <Button className="gap-2">
                  <Plus className="w-4 h-4" />
              ...
              <Card key={alert.id} className="border-border/50">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <div className={`w-3 h-3 rounded-full ${getAlertStatusColor(alert)}`} />
                ...
                <CardContent className="pt-0">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      <p className="text-muted-foreground font-medium">{t("alerts.triggerTime")}</p>
                      <p>{format(alert.triggerTime, "MMM dd, yyyy HH:mm")}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground font-medium">{t("alerts.createdAt")}</p>
                      <p>{formatDistanceToNow(alert.createdAt, { addSuffix: true })}</p>
                    </div>
                  </div>
                    ...
                    <div className="mt-4 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-md">
                      <div className="flex items-center gap-2 text-red-700 dark:text-red-400">
                        ...
                        <span className="font-medium">
                          Alert triggered on {format(alert.triggeredAt, "MMM dd, yyyy 'at' HH:mm")}
                        </span>
                      </div>
                    </div>
                  )}
